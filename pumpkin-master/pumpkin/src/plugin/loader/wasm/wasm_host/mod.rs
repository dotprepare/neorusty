...
pub const PLUGIN_RUNTIME_AVAILABLE: bool = false;

#[cfg(feature = "plugin-runtime")]
pub const PLUGIN_RUNTIME_AVAILABLE: bool = true;
...
pub mod wit {
    pub mod v0_1 {
        use super::super::PluginInitError;

        pub struct Plugin;

        pub fn add_to_linker(_: &mut wasmtime::component::Linker<super::super::state::PluginHostState>) -> wasmtime::Result<()> {
            Ok(())
        }

        pub fn prepare_plugin(_: &wasmtime::component::InstancePre<super::super::state::PluginHostState>) -> wasmtime::Result<PluginPre<super::super::state::PluginHostState>> {
            Err(wasmtime::Error::msg("WASM plugin runtime is unavailable"))
        }

        pub async fn init_plugin(
            _: &wasmtime::Engine,
            _: PluginPre<super::super::state::PluginHostState>,
        ) -> Result<(super::super::WasmPlugin, crate::plugin::PluginMetadata), PluginInitError> {
            Err(PluginInitError::UnsupportedRuntime)
        }

        pub struct PluginPre<T>(std::marker::PhantomData<T>);

        impl<T> PluginPre<T> {
            pub fn new(_: wasmtime::component::InstancePre<T>) -> wasmtime::Result<Self> {
...
pub enum PluginInitError {
    ...
    #[error("WASM plugin runtime is unavailable in this build")]
    UnsupportedRuntime,
}
...
impl PluginRuntime {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self, PluginInitError> {
        ...
        {
            let _ = path;
            return Err(PluginInitError::UnsupportedRuntime);
        }
