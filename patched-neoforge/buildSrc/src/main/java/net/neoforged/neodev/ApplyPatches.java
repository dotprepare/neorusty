...
abstract class ApplyPatches extends DefaultTask {
    ...
    public void applyPatches() throws IOException {
        ...
        try {
            copyZipStrippingNtfsExtra(originalJar, cleanJar);
        } catch (Exception e) {
            getLogger().warn("Failed to strip NTFS extra data, using original JAR: {}", e.getMessage());
            Files.copy(originalJar, cleanJar);
        }
        ...
        try {
            ...
            var patchedJar = getPatchedJar().get().getAsFile().toPath();
            getLogger().lifecycle("Cleaning NTFS extra data from {}", patchedJar.getFileName());
            var cleanedOutput = patchedJar.resolveSibling("clean-output-" + patchedJar.getFileName());
            copyZipStrippingNtfsExtra(patchedJar, cleanedOutput);
            if (Files.size(cleanedOutput) == 0) {
                throw new RuntimeException("Cleaned JAR is empty, something went wrong");
            }
            Files.move(cleanedOutput, patchedJar, java.nio.file.StandardCopyOption.REPLACE_EXISTING);
            getLogger().lifecycle("Cleaned {}", patchedJar.getFileName());
        } finally {
            Files.deleteIfExists(cleanJar);
        }
    }

    private void copyZipStrippingNtfsExtra(Path source, Path target) throws IOException {
        ZipUtil.copyZipStrippingNtfsExtra(source, target);
    }
}
